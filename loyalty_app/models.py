...
class LoyaltyCard(models.Model):
    ...
    def generate_card_image(self):
        """Генерация изображения карты лояльности"""
        # Создаём изображение (размер в пикселях)
        img = Image.new('RGB', (800, 500), color=(20, 60, 110))
        draw = ImageDraw.Draw(img)

        # Загружаем шрифт (укажите путь к ttf-файлу в вашей системе)
        try:
        ...
        # Добавляем текст на карту
        draw.text((50, 100), "Карта лояльности", font=font_large, fill=(255, 255, 255))
        draw.text((50, 200), f"Имя: {self.user.user_first_name or self.user.first_name}",
                  font=font_medium, fill=(255, 255, 255))
        draw.text((50, 250), f"Фамилия: {self.user.user_last_name or self.user.last_name}",
                  font=font_medium, fill=(255, 255, 255))
        draw.text((50, 300), f"Номер карты: {self.card_number}",
                  font=font_medium, fill=(255, 255, 255))
        ...
        # Сохраняем изображение в поле card_image
    ...
    def save(self, *args, **kwargs):
        ...
        # Генерируем изображение только при создании или если его нет
